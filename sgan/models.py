...
def get_noise(shape, noise_type, use_cuda=0):
    if noise_type == 'gaussian':
        rand_ = torch.randn(*shape)
        if use_cuda == 1:
            rand_ = rand_.cuda()
        return rand_
    elif noise_type == 'uniform':
        rand_ = torch.rand(*shape).sub_(0.5).mul_(2.0)
        if use_cuda == 1:
            rand_ = rand_.cuda()
        return rand_
...
class SocialPooling(nn.Module):
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            # curr_end_pos = curr_end_pos.dataset
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        if num_layers > 1:
            self.num_layer_embedding = nn.Linear(encoder_h_dim, encoder_h_dim * self.num_layers)
        ...
        # Decoder Hidden
        if pooling_type:
            decoder_input_dim = encoder_h_dim + bottleneck_dim
        else:
            decoder_input_dim = encoder_h_dim

        if self.mlp_decoder_needed():
            mlp_decoder_context_dims = [
                decoder_input_dim, mlp_dim, decoder_h_dim - self.noise_first_dim
            ]
    ...
    def add_noise(self, _input, seq_start_end, user_noise=None):
        ...
        if user_noise is not None:
            ...
            z_decoder = get_noise(noise_shape, self.noise_type, self.use_cuda)
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        if self.num_layers == 1:
            ...
            final_encoder_h = self.encoder(obs_traj_rel)[-1, :, :]

        #torch.Size([1, 1819, 32])
        # final_encoder_h = self.encoder(obs_traj_rel)
        # Pool States
        if self.pooling_type:
            ...
            # Construct input hidden states for decoder   torch.Size([1819, 64])
        ...
        if self.num_layers > 1:
            decoder_h = self.num_layer_embedding(decoder_h).view(self.num_layers, -1, self.encoder_h_dim)
...
class TrajectoryDiscriminator(nn.Module):
    ...
    def forward(self, traj, traj_rel, seq_start_end=None):
        ...
        if self.num_layers > 1:
            classifier_input = classifier_input[-1, :, :]
