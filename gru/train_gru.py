...
from losses_gru import l2_loss
from losses_gru import displacement_error, final_displacement_error
...
writer = SummaryWriter('./run/usa_random_10_10_lstm_1')
...
def main(args):
    ...
    print(get_dset_path(args.dataset_name, 'train'))
    ...
    gru.type(float_dtype).train()
    logger.info('Here is the gru:')
    logger.info(gru)


    optimizer = optim.Adam(gru.parameters(), lr=args.g_learning_rate)
    ...
    if restore_path is not None and os.path.isfile(restore_path):
        ...
        gru.load_state_dict(checkpoint['state'])
    ...
    while t < args.num_iterations:
        ...
        for batch in train_loader:
            ...
            losses = generator_step(args, batch, gru, optimizer)
            ...
            if t > 0 and t % args.checkpoint_every == 0:
                ...
                metrics_val = check_accuracy(
                    args, val_loader, gru, is_train=False
                )
                logger.info('Checking stats on train ...')
                metrics_train = check_accuracy(
                    args, train_loader, gru, limit=True, is_train=True
                )
                ...
                if metrics_val['ade'] == min_ade:
                    ...
                    checkpoint['best_t'] = t
                    checkpoint['best_state'] = gru.state_dict()

                # Save another checkpoint with model weights and
                # optimizer state
                checkpoint['state'] = gru.state_dict()
...
def generator_step(
    ...
    if args.clipping_threshold_g > 0:
        nn.utils.clip_grad_norm_(
            lstm.parameters(), args.clipping_threshold_g
        )
    optimizer.step()
